...
把 VS Code 中复制出来的 Codex 聊天回复持续收集成一张 PNG，并在每次变更后自动尝试复制到系统剪贴板。
...
7. 每次收集列表发生变化后，插件都会自动重新生成 PNG，并尝试复制到系统剪贴板。
8. 你可以直接去目标位置粘贴图片。
...
- `codexChatScreenshot.autoCopyOnChange`
  - 默认 `true`
  - 每次收集内容变化后，是否自动尝试复制最新 PNG
...
- 自动复制 PNG 依赖 webview 中的浏览器剪贴板能力；若受系统或环境限制，可以改用 `复制 PNG` 或 `下载 PNG`。
...
4. 自动复制 PNG 是否在你的系统和 VS Code 环境下生效。
